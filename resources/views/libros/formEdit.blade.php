...
<body>
    ...
    <h1>Editar libro</h1>
    <form action="/libros/{{$libro->id}}" method="POST"> {{--Editar--}} 
        @method('PATCH')
        @csrf
        <label for="titulo">Titulo</label><br>
        <input type="text" name="titulo" value="{{isset($libro)? $libro->titulo:''}}{{old('titulo')}}"><br>
        @error('titulo')
            <div class="alert alert-danger">{{$message}}</div>
        @enderror <br>
        <label for="autor">Autor</label><br>
        <input type="text" name="autor" value="{{isset($libro)? $libro->autor: ''}}{{old('autor')}}"><br>
        @error('autor')
            <div class="alert alert-danger">{{$message}}</div>
        @enderror <br>
